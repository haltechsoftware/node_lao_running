    ...
    bcrypt
      ...
      .then((encryptedPassword) => {
        packages.forEach((pack) =>
          createPackageResult(10, pack, encryptedPassword),
        );
      })
      .catch((error) => {
...
function createPackageResult(count, pack, encryptedPassword) {
  ...
  function userCreatedState(user) {
    ...
    fakerBuilder.forEach(() => {
      runnerResultFakers.push(
        user.createRunResult({
          ...
          range: (Math.random() * (5 - 0.02) + 0.02).toFixed(2),
    ...
    Promise.all(runnerResultFakers).then((runnerResults) => {
      const summary = runnerResults.reduce(
        (pre, curr) => {
          const total_range =
            parseFloat(pre.total_range) + parseFloat(curr.dataValues.range);
  ...
  Promise.all(userFakers).then((users) =>
    users.forEach((user) => userCreatedState(user)),
  );
}
